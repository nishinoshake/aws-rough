<template>
  <ServiceTemplate service-name="lambda">
    <section class="section">
      ...
      <p class="text">
        <a href="#サーバーレス" class="text-link">サーバーレス</a
        >でコードを実行できるサービスです。<br />現在は、Node.js/Python/Java/C#/Goがサポートされています
      </p>
      <p class="text">
        リクエスト数とメモリ使用量で料金が決まりますが、かなり寛大な無料枠があるので、軽く使う分には無料枠に収まると思います。
      </p>
    </section>
    <section class="section">
    ...
    <section class="section">
      ...
      <section class="section-child" id="サーバーレス">
        ...
        <p class="text">
          Lambdaを使用した場合、EC2のようにサーバーを構築する手間がなく、関数をアップして実行するだけなので、コードを書くことに集中できます。もちろん、サーバーが存在しない訳ではなく、雲の向こうの誰かが管理してくれているだけなので、感謝を忘れてはいけない。
        </p>
        <p class="text">
          サーバーレスと一言にいっても、<ExternalLink href="https://aws.amazon.com/jp/serverless/"
            >アーキテクチャー</ExternalLink
          >や、<ExternalLink href="https://aws.amazon.com/jp/lambda/">Lambda</ExternalLink>や<ExternalLink
            href="https://aws.amazon.com/jp/dynamodb/"
            >DynamoDB</ExternalLink
          >などのサービス、<ExternalLink href="https://serverless.com/">serverless</ExternalLink
          >という名前のフレームワークもあるので、少々ややこしいが慣れるしかない。
        </p>
        <p class="text">
          サーバーレスでシステムを構築すると、サーバーの事は意識しなくてよくなるが、サーバーレスであることを意識する必要が生じる。ここから先は哲学の領域。
        </p>
      </section>
    </section>
  </ServiceTemplate>
